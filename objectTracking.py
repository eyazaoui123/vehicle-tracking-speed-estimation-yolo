...
#asign a random unique id for each detected object
...
while True:
    ...
    if ret:
        ...
        #persis: tells tracker it should expect tracks from the previous frame
        results = model.track(frame, persist=True)
        #Visualize the results on the frame
        annotated_frame = results[0].plot()
        #Display the annotated frame
        cv2.imshow("YOLO11 Tracking", annotated_frame)
        #1 ms delay between consecutive frames and Break the loop if 'q' key is pressed
